...
say "\nNot koha-plugin-edifact-findaway, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-findaway";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-findaway';
    next if $repo eq 'koha-plugin-edifact-findaway-docs';
